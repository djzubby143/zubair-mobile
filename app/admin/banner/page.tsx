  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        {banner.layoutStyle === "full_width" ? (
          <div className="relative overflow-hidden rounded-2xl bg-slate-950 text-white border-2 border-[#dc2626]/40 shadow-lg aspect-[16/8] sm:aspect-[21/8] md:max-h-[320px] min-h-[180px] flex items-center justify-center">
            {/* Ambient Blurred Backdrop */}
            <img
              src={banner.imageUrl || "/logo.jpg"}
              alt=""
              aria-hidden="true"
            ...
            />
            {/* Foreground Fit to Screen Image */}
            <img
              src={banner.imageUrl || "/logo.jpg"}
              alt="Preview"
              className={`relative z-10 transition-transform duration-500 ${
                (banner.imageFit ?? "contain") === "contain"
            ...
            {banner.title && (
              <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-4 flex items-end justify-between gap-3 pointer-events-none">
                <div className="space-y-0.5 max-w-lg">
                  {banner.badge && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#dc2626] text-white text-[9.5px] font-black uppercase">
                      {banner.badge}
                    </span>
                  )}
                  <h3 className="text-sm sm:text-base font-black text-white truncate drop-shadow">
                    {banner.title}
                  </h3>
                </div>
                {banner.buttonText && (
                  <div className="inline-flex items-center gap-1.5 bg-[#25D366] text-white text-[11px] font-black px-3 py-1.5 rounded-lg shrink-0 shadow">
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#111827] via-[#1f2937] to-[#111827] text-white border-2 border-[#dc2626]/40 shadow-lg">
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 p-5 sm:p-7">
              ...
              {banner.imageUrl && (
                <div className="w-full md:w-80 lg:w-[360px] h-48 sm:h-56 rounded-xl overflow-hidden border border-slate-700/80 shadow-md shrink-0 relative bg-slate-950 flex items-center justify-center">
                  {/* Ambient Backdrop */}
                  <img
                    src={banner.imageUrl}
                    alt=""
                  ...
                  <div className="absolute bottom-2.5 left-3 right-3 text-white text-[11px] font-bold truncate z-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => setBanner({ ...banner, imageFit: "contain" })}
                  className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                    (banner.imageFit ?? "contain") === "contain"
                      ? "border-[#dc2626] bg-red-50/80 ring-2 ring-red-400/20 shadow-2xs"
                      : "border-slate-200 bg-white hover:border-slate-300"
                  }`}
                >
                  <div className="text-xs font-bold text-slate-900">
                    Fit to Screen (تجویز کردہ)
                  </div>
                  <p className="text-[10.5px] text-slate-500 mt-1 leading-snug">
                    پورا امیج فریم میں فِٹ رہے گا۔ سائز بڑا پھیل کر خراب نہیں ہوگا اور نہ ہی کٹنگ ہوگی۔
                  </p>
                </button>

                <button
                  type="button"
                  onClick={() => setBanner({ ...banner, imageFit: "cover" })}
                  className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                ...
                >
                  ...
                  <p className="text-[10.5px] text-slate-500 mt-1 leading-snug">
                    پورا باکس بھر دے گا، لیکن کنارے کٹ سکتے ہیں۔
                  </p>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => setBanner({ ...banner, layoutStyle: "split" })}
                  className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                    (banner.layoutStyle ?? "split") === "split"
                      ? "border-[#dc2626] bg-red-50/80 ring-2 ring-red-400/20 shadow-2xs"
                      : "border-slate-200 bg-white hover:border-slate-300"
                  }`}
                >
                  ...
                  <p className="text-[10.5px] text-slate-500 mt-1 leading-snug">
                    بائیں طرف تحریر، بٹن اور دائیں طرف امیج سلائیڈر۔
                  </p>
                </button>

                <button
                  type="button"
                  onClick={() => setBanner({ ...banner, layoutStyle: "full_width" })}
                  className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                    banner.layoutStyle === "full_width"
                      ? "border-[#dc2626] bg-red-50/80 ring-2 ring-red-400/20 shadow-2xs"
                      : "border-slate-200 bg-white hover:border-slate-300"
                ...
                >
                  ...
                  <p className="text-[10.5px] text-slate-500 mt-1 leading-snug">
                    پوری چوڑائی پر پھیلا ہوا بڑا بینر سلائیڈر۔
                  </p>
                </button>
              </div>
            </div>
                ...
                {(banner.images && banner.images.length > 0 ? banner.images : [banner.imageUrl || "/logo.jpg"]).map(
                  (imgUrl, idx) => (
                    ...
                    >
                      ...
                      <div className="h-24 w-full rounded-lg overflow-hidden bg-slate-950 border border-slate-200 mb-2 relative flex items-center justify-center">
                        <img
                          src={imgUrl}
                          alt=""
                          aria-hidden="true"
                        ...
                        <img
                          ...
                          className="relative z-10 max-w-full max-h-full w-auto h-auto object-contain p-1 group-hover:scale-105 transition-transform duration-300"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = "/logo.jpg";
                          }}
                        />
