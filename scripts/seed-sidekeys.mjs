...
// Helper for model name extraction from "SideKey Model"
...
function createSlug(base, existingSlugs) {
  let slug = base
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9]+/g, "-")
...
// Generate unique SKU
function createSku(modelName, index, existingSkus) {
...
async function main() {
  ...
  try {
    // Check if category with slug 'side-keys' or 'sidekey' exists
    const { data: existingCats, error: catSearchErr } = await supabase
      .from("categories")
      .select("id, name, slug")
    ...
    console.warn("⚠️ Category check notice:", err.message);
    categoryId = "f1f0fb31-2b9b-4666-9149-ea669e417a8e";
  }
  ...
  const sanitizedProducts = rawRows.map((row, idx) => {
    // Read name from 'Item Name' or 'Item name*'
    const rawName = String(row["Item Name"] || row["Item name*"] || row["name"] || "").trim();
    const model = extractModelName(rawName);

    // Read and sanitize price
    ...
    // Current Stock (negative numbers like -2, -5 sanitized to 0)
  ...
  console.log(`🔍 Sample item 1: ${JSON.stringify(sanitizedProducts[0], null, 2)}`);
  console.log(`🔍 Sample item 2: ${JSON.stringify(sanitizedProducts[1], null, 2)}`);
  ...
  for (let i = 0; i < sanitizedProducts.length; i += BATCH_SIZE) {
    ...
    try {
      // Clean undefined fields for Supabase
      ...
      if (error) {
        ...
        // Try fallback without purchase_price column if missing remotely
      ...
      console.error(`❌ Batch ${batchNum}/${totalBatches} exception:`, batchErr.message);
      totalFailed += batch.length;
    }
  }
...
function generateSqlSeed(categoryId, products) {
  let sql = `-- ==========================================================\n`;
